"""
Wind Forecasting — Training Pipeline
========================================
Same structure as demand/solar trainers, specialised for wind generation.
"""
...
log = get_module_logger("ml_forecasting.wind.trainer")

FORECAST_TYPE = "wind"
MODEL_FILENAME = "wind_xgboost.joblib"
...
def train_wind_model(
    ...
    """
    End-to-end training pipeline for the wind forecaster.
    ...
    log.info(f"Loaded {len(data)} rows for wind training")
    ...
    residuals_path = save_dir / "wind_residuals.npy"
    np.save(residuals_path, residuals)

    log.info(f"Wind test metrics - MAE: {mae:.2f} kW, RMSE: {rmse:.2f} kW, MAPE: {mape:.2f}%")
    ...
    return {
        ...
        "message": f"Wind model trained and saved to {model_path}",
        "residuals": residuals,
    }
